...
def process_text_data(
    ...
    if user_summary_merge_vector and item_summary_vector:
        ...
        for user in tqdm(df_["user_id"].unique()):
            vector = text_to_vector(
                summary_merge(df_, user, 5), tokenizer, model, device
            )
        ...
        if train == True:
            user = np.load(
                "data/text_vector/train_user_summary_merge_vector.npy",
                allow_pickle=True,
            )
        else:
            user = np.load(
                "data/text_vector/test_user_summary_merge_vector.npy", allow_pickle=True
            )
        ...
        if train == True:
            item = np.load(
                "data/text_vector/train_item_summary_vector.npy", allow_pickle=True
            )
        else:
            item = np.load(
                "data/text_vector/test_item_summary_vector.npy", allow_pickle=True
            )
    ...
    df_ = pd.merge(
        df_, books_text_df[["isbn", "item_summary_vector"]], on="isbn", how="left"
    )
    return df_
...
class Text_Dataset(Dataset):
    def __init__(
        self, user_isbn_vector, user_summary_merge_vector, item_summary_vector, label
    ):
        """
        Parameters
        ----------
    ...
    def __getitem__(self, i):
        return {
            "user_isbn_vector": torch.tensor(
                self.user_isbn_vector[i], dtype=torch.long
            ),
            "user_summary_merge_vector": torch.tensor(
                self.user_summary_merge_vector[i].reshape(-1, 1), dtype=torch.float32
            ),
            "item_summary_vector": torch.tensor(
                self.item_summary_vector[i].reshape(-1, 1), dtype=torch.float32
            ),
...
def text_data_load(args):
    ...
    text_train = process_text_data(
        ...
        args.device,
        train=True,
        user_summary_merge_vector=args.vector_create,
        item_summary_vector=args.vector_create,
    )
